...
const MenuContainer: React.FC<PropsType> = (props) => {
  ...
  function OnOpenExtraOptions(i: OrderType) {
    const defaultCheckedOption = extraMenu?.find(
      ({ id }: any) => id === "default"
    );
    setIsOpen(true);
    setSecondMenu(i);
  ...
  const AddToCart = (order: OrderType) => {
    const orderWithOption = {
      ...order,
      id: v1(),
      weight: totalItemWeight || order.weight,
      price: FixPriceDecimals(pricer),
      option: stateOptions || localStateOptions,
    };
  ...
  const Option = ({ option }: any) => {
    const findCheckedOption = stateOptions?.find(
      ({ id }: any) => id === option.id
    );
    function OnChangeOptionCheckBoxType(option: OptionType) {
      if (findCheckedOption?.id !== option.id) {
        ...
        setPricer(
          secondMenu.price < pricer
            ? (prev: any) => prev - option.price
            : secondMenu.price
        );
        setTotalItemWeight(
          secondMenu.weight < totalItemWeight
            ? (prev: any) => prev - option.weight
            : secondMenu.weight
        );
    ...
    function OnChangeOptionRadioType(option: OptionType) {
      if (checked !== option.id) {
        ...
        setTotalItemWeight(
          secondMenu.weight > option.weight
            ? secondMenu.weight + option.weight
            : option.weight
        );
    ...
    return (
      <div className={styles.optionsContainer}>
        {option.multiply ? (
          <div
            onClick={() => OnChangeOptionCheckBoxType(option)}
            className={
              findCheckedOption?.id === option.id
                ? styles.check
                : styles.options
            }
          >
            <div className={styles.imageAndNameContainer}>
              {option.img ? (
                <img className={styles.img} src={option.img} alt="i.name" />
              ) : (
                <></>
              )}
              <div>{option.name}</div>
            ...
            <div className={styles.weightAndPriceContainer}>
              <div className={styles.weight}>+{option.weight}g</div>
              <div className={styles.price}>${option.price}</div>
            </div>
          </div>
        ) : (
          <div
            onClick={() => OnChangeOptionRadioType(option)}
            className={option.id === checked ? styles.check : styles.options}
          >
            <div className={styles.imageAndNameContainer}>
              {option.img ? (
                <img className={styles.img} src={option.img} alt="i.name" />
